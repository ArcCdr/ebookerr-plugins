...
def test_text_download_source_manifest_matches_the_in_image_manifest() -> None:
    ...
    assert parsed == dataclasses.replace(TextDownloadSourcePlugin.manifest, transport="local_exec")


def test_the_manifest_opts_into_update_checks_and_its_format() -> None:
    """The Source answers the Auto-Pull update check and claims the ``txt`` format out of process."""
